  ...
  (with-handler hello-handler
    (let [url (str "http://localhost:" port)
          ...
          r2 @(http/get url {:save-request? true
                             :debug-body? false})]
      (is (contains? r1 :iny/request))
      (is (= "name=John" (get-in r1 [:iny/request :query-string])))
      (is (contains? r1 :iny/netty-request))
      (is (contains? r1 :iny/request-body))
      (is (not (contains? r2 :iny/request-body))))))
