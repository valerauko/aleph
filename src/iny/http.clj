...
(defn start-server
  "Starts an HTTP server using the provided Ring `handler`.  Returns a server object which can be stopped
   via `java.io.Closeable.close()`, and whose port can be discovered with `iny.netty/port`.
...
(defn connection-pool
  "Returns a connection pool which can be used as an argument in `request`.
   ...
   | `dns-options` | an optional map with async DNS resolver settings, for more information check `iny.netty/dns-resolver-group`. When set, ignores `name-resolver` setting from `connection-options` in favor of shared DNS resolver instance
   | `middleware` | a function to modify request before sending, defaults to `iny.http.client-middleware/wrap-request`
  ...
  (let [log-activity (:log-activity connection-options)
        ...
        conn-options' (cond-> connection-options
                        ...
                        (some? log-activity)
                        (assoc :log-activity (netty/activity-logger "iny-client" log-activity)))
        p (promise)
...
(let [maybe-timeout! (fn [d timeout] (when d (d/timeout! d timeout)))]
  (defn request
    "Takes an HTTP request, as defined by the Ring protocol, with the extensions defined
     ...
     | `follow-redirects?` | whether to follow redirects, defaults to `true`; see `iny.http.client-middleware/handle-redirects`"
    [{:keys [pool
             middleware
             pool-timeout
             response-executor
...
(defn get-all
  ...
  [^iny.http.core.HeaderMap headers ^String k]
  (-> headers ^io.netty.handler.codec.http.HttpHeaders (.headers) (.getAll k)))

(defn wrap-ring-async-handler
  "Converts given asynchronous Ring handler to iny-compliant handler.
