(ns iny.http.client
  (:require
    ...
    [iny.http.multipart :as multipart]
    [iny.netty :as netty])
  (:import
    [java.io
     IOException]
  ...
  (fn [^ChannelPipeline pipeline]
    (let [handler (if raw-stream?
          ...
          logger (cond
                   ...
                   (some? log-activity)
                   (netty/activity-logger "iny-client" log-activity)
            ...
            (try
              (let [proxy-options' (when (some? proxy-options)
                ...
                (let [body (:body req)
                  ...
                  (when-let [save-message (get req :iny/save-request-message)]
                    ;; debug purpose only
                  ...
                  (when-let [save-body (get req :iny/save-request-body)]
