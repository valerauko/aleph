...
(p/def-derived-map NettyResponse [^HttpResponse rsp complete body]
  ...
  :iny/keep-alive? (HttpUtil/isKeepAlive rsp)
  :headers (-> rsp .headers headers->map)
  :iny/complete complete
  :body body)
...
(defn netty-request->ring-request [^HttpRequest req ssl? ch body]
  (assoc
    ...
    :iny/request-arrived (System/nanoTime)))
      ...
      (let [d (d/deferred)]
        (s/on-closed sink
          (fn []
            ...
            (.execute (-> ch iny.netty/channel .eventLoop)
              #(d/success! d
...
(defn send-contiguous-body [ch ^HttpMessage msg body]
  (let [omitted? (identical? :iny/omitted body)
...
(defn websocket-ping [conn d' data]
  (d/chain'
   (s/put! conn (iny.http.core.WebsocketPing. d' data))
...
(defn websocket-close! [conn status-code reason-text d']
  ...
  (let [payload (iny.http.core/WebsocketClose. d' status-code reason-text)]
    (d/chain'
     (s/put! conn payload)
     (fn [put?]
