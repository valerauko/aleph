...
(defn extract-cookies-from-response-headers
  ...
  ([cookie-spec ^iny.http.core.HeaderMap headers]
...
(defn wrap-request-debug [req]
  (cond-> req
    ...
    (assoc :iny/save-request-message (atom nil))

    (opt req :debug-body)
    (assoc :iny/save-request-body (atom nil))))

(defn handle-response-debug [req rsp]
  (let [saved-message (get req :iny/save-request-message)
        saved-body (get req :iny/save-request-body)
        req' (dissoc req
                     :iny/save-request-body
                     :iny/save-request-message
                     :save-request
                     :save-request?
                     :debug-body
                     :debug-body?)]
    (cond-> rsp
      (some? saved-message)
      (assoc :iny/netty-request @saved-message)

      (some? saved-body)
      (assoc :iny/request-body @saved-body)

      (opt req :save-request)
      (assoc :iny/request req'))))
