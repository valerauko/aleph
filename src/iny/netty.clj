  ...
  (when f
    (if (.isSuccess f)
      ...
      (let [d (d/deferred nil)
            ...
            ;; https://github.com/ztellman/iny/issues/365
...
(defmacro safe-execute [ch & body]
  `(let [f# (fn [] ~@body)
         event-loop# (-> ~ch iny.netty/channel .eventLoop)]
...
(defn sink
  ...
  ([ch downstream? coerce-fn additional-description]
    (let [sink (->ChannelSink
      ...
      (doto sink (reset-meta! {:iny/channel ch})))))
               ...
               (fn [m]
                 (assoc m
                   ...
                   :connection (assoc (connection-stats ch true)
                                 :direction :inbound)))})]
    (doto src (reset-meta! {:iny/channel ch}))))
...
(defn instrument!
  [stream]
  (if-let [^Channel ch (->> stream meta :iny/channel)]
    (do
      (safe-execute ch
...
(def ^String client-event-thread-pool-name "iny-netty-client-event-pool")
...
(defn start-server
  ...
  (let [num-cores      (.availableProcessors (Runtime/getRuntime))
        ...
        thread-factory (enumerating-thread-factory "iny-netty-server-event-pool" false)
        closed?        (atom false)
    ...
    (try
      (let [b (doto (ServerBootstrap.)
        ...
        (reify
          ...
          Object
          (toString [_]
            (format "inyServer[channel:%s, transport:%s]" ch transport))
          inyServer
          (port [_]
